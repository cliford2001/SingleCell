...
The table below records the versions available in the active Docker container
used for this methods draft.
...
![Pre-filter QC violin plots.](assets/qc_prefilter_generic.png){width=95%}
...
![Post-filter QC violin plots.](assets/qc_postfilter_generic.png){width=95%}
...
![UMAP before Harmony correction, colored by sample.](assets/umap_preharmony_generic.png){width=95%}
...
![UMAP after Harmony correction, colored by sample.](assets/umap_postharmony_generic.png){width=95%}
...
![Elbow diagnostic used to inspect candidate cluster numbers.](assets/elbow_plot_generic.png){width=85%}

![Cluster tree across candidate Leiden resolutions.](assets/clustree_generic.png){width=95%}
...
![Final UMAP colored by numbered Seurat clusters.](assets/umap_seuratclusters_generic.png){width=95%}
...
![Marker dotplot after bibliography-based cell-type annotation.](assets/dotplot_annotation_biblio_generic.png){width=95%}

![UMAP colored by bibliography-based cell-type annotation.](assets/umap_annotation_biblio_generic.png){width=95%}
...
![Cluster tree labeled by the dominant bibliography-based annotation in each node.](assets/clustree_annotated_generic.png){width=95%}
...
![Violin plot for the selected gene across all annotated cell types.](assets/vln_gene_all_generic.png){width=95%}

![UMAP expression overlay for the selected gene.](assets/feature_gene_all_generic.png){width=95%}
...
![UMAP colored by grouped cell-type labels.](assets/umap_grouped_generic.png){width=95%}
...
![Example subcluster UMAP used for manual marker inspection.](assets/subcluster_mesophyll_generic.png){width=95%}
...
### Minimum components to run on a personal computer

The minimum practical setup for reproducing Chapter 1 on a local workstation is:
...
recommended. Cell Ranger is especially CPU- and memory-intensive and is not
installed inside the downstream `matigara/scrnaseq:latest` Docker image.
...
knitr::kable(r_versions, caption = "Complete R package inventory in the Docker image.")
```
