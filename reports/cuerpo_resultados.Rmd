...
![Pre-filter QC violin plots.](../resultados/01_qc/qc_prefilter.pdf){width=30%}
...
![Post-filter QC violin plots.](../resultados/01_qc/qc_postfilter.pdf){width=30%}
...
![UMAP before Harmony correction, colored by sample.](../resultados/01_qc/umap_preharmony.pdf){width=30%}
...
![UMAP after Harmony correction, colored by sample.](../resultados/01_qc/umap_postharmony.pdf){width=30%}
...
\begin{center}
```
![Elbow diagnostic used to inspect candidate cluster numbers.](../resultados/02_clustering/elbow_plot.pdf){width=30%} ![Cluster tree across candidate Leiden resolutions.](../resultados/02_clustering/clustree2.pdf){width=30%}
```{=latex}
\end{center}
```
...
![Final UMAP colored by numbered Seurat clusters.](../resultados/02_clustering/umap_seuratclusters.pdf){width=30%}
...
\begin{center}
```
![Marker dotplot after bibliography-based cell-type annotation.](../resultados/03_annotation/dotplot_marker_table_annotation_biblio.pdf){width=30%} ![UMAP colored by bibliography-based cell-type annotation.](../resultados/03_annotation/umap_annotation_biblio.pdf){width=30%}
```{=latex}
\end{center}
```
...
![Cluster tree labeled by the dominant bibliography-based annotation in each node.](../resultados/03_annotation/clustree_annotated.pdf){width=30%}
...
\begin{center}
```
![Violin plot for the selected gene across all annotated cell types.](../resultados/04_expression/vln_gene_AT5G26000.pdf){width=30%} ![UMAP expression overlay for the selected gene.](../resultados/04_expression/feature_gene_AT5G26000.pdf){width=30%}
```{=latex}
\end{center}
```
...
# Merge the two mesophyll subclusters into a single "Mesophyll" label
grouping <- c(
  "Mesophyll.1" = "Mesophyll",
  "Mesophyll.2" = "Mesophyll"
)
...
![UMAP colored by grouped cell-type labels.](assets/umap_grouped_mesophyll_docker.pdf){width=30%}
...
# export_to_scanpy(
#   subset(pbmc_harmony, subset = celltype_curated == "GC"),
...
![Volcano plot for Mesophyll_1, WT vs pifq.](../resultados/06_de_results/WT_vs_pifq/volcano/DESeq2_Mesophyll_1_WT_vs_pifq.png){width=30%}
...
![GO enrichment (BP) for Mesophyll_1, WT vs pifq.](../resultados/07_go/WT_vs_pifq/GO_Mesophyll_1_WT_vs_pifq_WT_vs_pifq_BP_bubble.pdf){width=30%}
...
![log2FC heatmap: rows hierarchically clustered, columns in marker-table order (unannotated clusters last), WT vs pifq.](../resultados/06_de_results/WT_vs_pifq/heatmap_WT_vs_pifq.pdf){width=30%}
...
![TF co-expression network, colored by DE direction.](../resultados/08_networks/network_tf_DE_direction.pdf){width=30%}
...
![Overview UMAP colored by curated cell type.](../resultados/09_pseudotime/umap_overview.pdf){width=30%}

## Section 25 - Cell type selection

Picks the cell types used to build the trajectory. Here we follow the
epidermis lineage alone (the `Epidermis` label; guard cells are a separate
`GC` group and are not included here). Names must match the Section 24 list
exactly.
...
![UMAP of the cells selected for the trajectory.](../resultados/09_pseudotime/umap_selection.pdf){width=30%}
...
Representative outputs from Section 26. `sigma=0.3` was chosen (over the
default 0.1) after a one-at-a-time parameter sweep (nodes/sigma/lambda/eigs)
around the same root cell, for a looser tree fit:

```{=latex}
\begin{center}
```
![Trajectory graph colored by cell type.](../resultados/09_pseudotime/trajectory/n50_s0.3_l200_e8_seed3/n50_s0.3_l200_e8_seed3_annotation.pdf){width=30%} ![Pseudotime overlaid on the force-directed layout.](../resultados/09_pseudotime/trajectory/n50_s0.3_l200_e8_seed3/n50_s0.3_l200_e8_seed3_pseudotime_trajectory.pdf){width=30%}
```{=latex}
\end{center}
```

![Root cell (red) automatically selected within the Epidermis cluster as the most-connected cell — pseudotime = 0 here.](../resultados/09_pseudotime/trajectory/n50_s0.3_l200_e8_seed3/n50_s0.3_l200_e8_seed3_root_cell.pdf){width=30%}
...
![Selected genes on the trajectory layout.](../resultados/09_pseudotime/trajectory/n50_s0.3_l200_e8_seed3/gene_plots/n50_s0.3_l200_e8_seed3_gene_plots.pdf){width=30%}
...
\begin{center}
```
![Top 10 variable genes along pseudotime.](../resultados/09_pseudotime/trajectory/n50_s0.3_l200_e8_seed3/gene_trends/step29_gene_trends_top10_FINAL_24x18.pdf){width=48%} ![Highlighted gene list along pseudotime.](../resultados/09_pseudotime/trajectory/n50_s0.3_l200_e8_seed3/gene_trends/step29_gene_trends_highlight_list_FINAL_24x18.pdf){width=48%}
```{=latex}
\end{center}
```
